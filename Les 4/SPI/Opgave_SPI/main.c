...
void spi_masterInit(void)
{
	...
	SPCR = 0xD2;							// Enable spi, MasterMode, Clock rate fck/64, bitrate=125kHz
...
void spi_write_specific(int displayNmr, unsigned char data)
{
	spi_write(displayNmr);
	spi_write(data);
}
...
void spi_setIntensity(unsigned char intensity)
{
	spi_slaveSelect(0);   //Select the display chip
	spi_write(0x0A);	  //Register 0A for the intensity
	spi_write(intensity); //Set a level between 0x00 en 0x0F
	spi_slaveDeSelect(0); //Deselect the slave
}
